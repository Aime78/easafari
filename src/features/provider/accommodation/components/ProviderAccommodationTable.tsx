...
const ProviderAccommodationTable = ({
  accommodations,
  selectedCategory,
  ...
  const getFilteredAccommodations = () => {
    ...
    if (selectedCategory) {
      filtered = filtered.filter(
        (acc) => acc.accommodation_category_id === selectedCategory.id
      );
    }
    ...
    if (searchTerm.trim()) {
      filtered = filtered.filter(
        (acc) =>
          ...
          acc.amenities.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    return filtered;
  };
        ...
        <p className="text-gray-500 text-center max-w-sm mb-6">
          {isFiltered ? (
            searchTerm.trim() ? (
              selectedCategory ? (
                <>
                  No accommodations in {selectedCategory.name} match "
                  {searchTerm}
                  ". Try different keywords or browse other categories.
                </>
              ) : (
                <>
                  No accommodations match your search "{searchTerm}". Try
                  different keywords or browse categories.
                </>
              )
            ) : (
              <>
                No accommodations found in {selectedCategory?.name}. Try
                browsing other categories.
              </>
            )
          ) : (
            "Get started by adding your first accommodation to showcase amazing places to stay."
          )}
        </p>
        {!isFiltered && (
  ...
  return (
    <Card className="p-6 border-none shadow-none">
                      ...
                      <div className="flex items-center">
                        {acc.rating ? (
                          <>
                            ...
                            <span>
                              {acc.rating ? acc.rating.toFixed(1) : "N/A"}
                            </span>
                          </>
                        ) : (
            ...
            <div className="flex items-center justify-between px-2 py-4">
              ...
              <div className="flex items-center space-x-6 lg:space-x-8">
                ...
                <div className="text-sm text-muted-foreground">
                  ...
                  {Math.min(endIndex, filteredAccommodations.length)} of{" "}
      ...
      {accommodationToDelete && (
        <DeleteProviderAccommodationDialog
...
export default ProviderAccommodationTable;
